...
public class Texture implements AutoCloseable {
	...
	public Texture(int width, int height, boolean calloc) {
		this(Format.RGBA, width, height, calloc);
	}

	public Texture(Format format, int width, int height, boolean calloc) {
		this.format = format;
		this.width = width;
		this.height = height;
	...
	private Texture(Format format, int width, int height, boolean useStbFree, long addr) {
		this.format = format;
		this.width = width;
		this.height = height;
	...
	public static Texture read(InputStream inputStream) throws IOException {
		return read(Format.RGBA, inputStream);
	}

	@SuppressWarnings("deprecation")
	public static Texture read(@Nullable Format format, InputStream inputStream) throws IOException {
	...
	public static Texture read(ByteBuffer byteBuffer) throws IOException {
		return read(Format.RGBA, byteBuffer);
	}

	public static Texture read(@Nullable Format format, ByteBuffer byteBuffer) throws IOException {
		...
		try (MemoryStack memoryStack = MemoryStack.stackPush()) {
			...
			return new Texture(format == null ? Format.getStbFormat(channel.get(0)) : format, width.get(0),
	...
	public void resizeSubRectTo(int x, int y, int inputW, int inputH, Texture texture) {
		checkAllocated();
		if (texture.format() != format)
	...
	public static Texture fromBase64(String base64) throws IOException {
		try (MemoryStack memoryStack = MemoryStack.stackPush()) {
